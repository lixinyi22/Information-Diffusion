...
class BaseRunner(object):
		...
		for k in topk:
			...
			for metric in metrics:
				...
				if metric == 'HR':
					...
					'''ap = np.zeros_like(gt_rank, dtype=np.float32)
					for i in range(len(gt_rank)):
						rank = gt_rank[i]
						if rank <= k:
							ap[i] = 1.0 / rank  # 这里假设只有一个相关项，AP = 1/rank
					evaluations[key] = ap.mean()  # MAP = AP 的平均'''
				else:
	...
	def predict(self, dataset: BaseModel.Dataset, save_prediction: bool = False) -> np.ndarray:
		...
		for batch in tqdm(dl, leave=False, ncols=100, mininterval=1, desc='Predict'):
			if hasattr(dataset.model,'inference'):
				...
				#i_gate = result['i_gate']
				#his_gate = result['his_gate']
			else:
				result = dataset.model(utils.batch_to_gpu(batch, dataset.model.device))
				#result2 = dataset.model.model_predict(utils.batch_to_gpu(batch, dataset.model.device))
				prediction = result['prediction']
				#prediction2 = result2['prediction']
				#i_gate = result['i_gate']
				#his_gate = result['his_gate']
			predictions.extend(prediction.cpu().data.numpy())
			#predictions2.extend(prediction2.cpu().data.numpy())
			#i_gates.extend(i_gate.cpu().data.numpy())
			#his_gates.extend(his_gate.cpu().data.numpy())
		predictions = np.array(predictions)
		#predictions2 = np.array(predictions2)
		#i_gates = np.array(i_gates)
		#his_gates = np.array(his_gates)
	...
	def print_res(self, dataset: BaseModel.Dataset) -> str:
		...
		#res_str2 = '(' + utils.format_metric(result_dict2) + ')'
		return res_str
