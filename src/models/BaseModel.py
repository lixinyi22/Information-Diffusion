...
class BaseModel(nn.Module):
	...
	class Dataset(BaseDataset):
		def __init__(self, model, corpus, phase: str ):
			...
			self.diffusion_graph = corpus.diffusion_graph
		...
		def collate_batch(self, feed_dicts: List[dict]) -> dict:
			...
			feed_dict['diffusion_graph'] = self.diffusion_graph
			return feed_dict
...
class GeneralModel(BaseModel):
	...
	def __init__(self, args, corpus):
		...
		self.loss_fn = nn.CrossEntropyLoss()  # for BPR loss
	...
	def loss(self, out_dict: dict) -> torch.Tensor:
		...
		targets = torch.zeros(predictions.size(0), dtype=torch.long, device=predictions.device)  # [batch_size], all 0
