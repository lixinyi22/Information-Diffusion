...
class DisenIDP(SequentialModel):
	extra_log_args = ['emb_size', 'num_layers', 'num_heads']
	reader = 'DisenIDPReader'
	...
	def _base_define_params(self, args):
		# additional layers
		self.long_term_att = Long_term_atention(input_size=self.emb_size, attn_dropout=self.dropout)
		self.gnn_diffusion_layer = LSTMGNN(self.emb_size, self.item_num, self.dropout)
		...
		self.short_term_att = Short_term_atention(input_size=self.emb_size, attn_dropout=self.dropout)
		self.linear = nn.Linear(self.emb_size*3, self.emb_size)
		self.past_multi_att = TransformerBlock(input_size=self.emb_size, n_heads=4, attn_dropout=self.dropout)
		self.ssl_weights = nn.ParameterList([nn.Parameter(torch.zeros(self.emb_size, self.emb_size)) for _ in range(2)])
		self.ssl_bias = nn.ParameterList([nn.Parameter(torch.zeros(1, self.emb_size)) for _ in range(2)])
	...
	def forward(self, feed_dict):
		...
		graph_user_embedding = self.gnn_diffusion_layer(diffusion_graph, feed_dict['phase'], self.device)
		...
		# DisenIDP-local
		'''source_emb = his_vectors[:, 0, :]
		L_cas_emb = self.long_term_att(source_emb, his_vectors, his_vectors, mask= mask.cuda())
		user_cas_gru, _ = self.past_gru(his_vectors)
		user_cas_lstm, _ = self.past_lstm(his_vectors)
		S_cas_emb = self.short_term_att(user_cas_gru, user_cas_lstm, user_cas_lstm, mask=mask.cuda())
		LS_cas_emb = torch.cat([his_vectors, L_cas_emb, S_cas_emb], -1)
		LS_cas_emb = self.linear(LS_cas_emb)
		his_vectors = self.past_multi_att(LS_cas_emb, LS_cas_emb, LS_cas_emb, mask)
		his_vector = his_vectors[torch.arange(batch_size), lengths - 1, :]'''
